var user_role = {
		...
		roleTree : {
			...
			loadRoleTree : function() {
				$.post("roleAction_showRoleByUid.action", {
							...
							user_role.data.zTreePlugin = $("#roleTree").zTree(
									user_role.opt.roleTree.setting, data);
			...
			saveRole : function() {
				// 得到被选中的权限
				var checkedNodes = user_role.data.zTreePlugin
				...
				$.post("roleAction_saveRole.action", paramter, function(data) {
							alert("保存成功");
						});
			}
		}
	...
	init : {
		...
		initData : function() {
			...
			var uid = $(this).parent().siblings("input[type='hidden']")
					.attr("value");
