var roletree = {
	...
	loadRoleTree : function() {
		$.post("roleAction_showRoleTree.action", null, function(data) {
					roletree.data.zTreePlugin = $("#roleTree").zTree(
							roletree.setting, data);
				});
	},
	// 增加角色
	addRole : function() {
		...
		if (roleName) {
			...
			$.post("roleAction_showRoleByName.action", {
						...
						if (data == "1") { // 可用
							...
							$.post("roleAction_add.action", parameter,
									function(data1) {
										var newNode = {
											rid : data1.rid,
											name : roleName,
										...
										// roletree.data.treeNode父节点
										roletree.data.zTreePlugin.addNodes(
												roletree.data.treeNode,
												newNode, true);
									});
						} else {
							alert("已经存在该角色");
	...
	deleteRole : function() {
		...
		if (window.confirm("您确认要删除吗?")) {
			$.post("roleAction_deleteRole.action", {
						...
						roletree.data.zTreePlugin
								.removeNode(roletree.data.treeNode);
					});
		}
	},
	// 修改角色
	updateRole : function() {
		...
		if (roletree.data.zTreePlugin.getNodeByParam("name", roleName)) { // 如果得到的对象存在,
																			// 则为true
			alert("该名称已经存在了")
			...
			$.post("roleAction_updateRole.action", {
						...
						// 获取当前点击的节点
						var node = roletree.data.zTreePlugin.getNodeByParam(
								"rid", roletree.data.treeNode.rid);
						// 修改其name属性
						node.name = roleName;
						// 刷新ztree
